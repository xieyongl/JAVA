...
public class FileUploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (isMultipart) {
			...
			if (isUploadSuccess) {
				// 文件上传成功，将信息保存到数据库中
				KinderDao kpd = new KinderDaoImpl();
				kpd.addKP(kp);
				// 返回所有的用户信息到主界面显示
				ArrayList<Kinder> kps = kpd.getALLKinder();
				request.setAttribute("userphotos", kps);
				request.getRequestDispatcher("/showuserphoto.jsp").forward(request, response);
			} else {
